...
OBX_C_API obx_err
obx_sync_credentials(OBX_sync *sync, OBXSyncCredentialsType type, const void *data, size_t size);
...
OBX_C_API obx_err
obx_sync_outgoing_message_count(OBX_sync *sync, uint64_t limit, uint64_t *out_count);
...
OBX_C_API OBX_sync_msg_objects_builder *
obx_sync_msg_objects_builder(const void *topic, size_t topic_size);
...
OBX_C_API obx_err
obx_sync_msg_objects_builder_add(OBX_sync_msg_objects_builder *message, OBXSyncObjectType type,
...
OBX_C_API void
obx_sync_listener_connect(OBX_sync *sync, OBX_sync_listener_connect *listener, void *listener_arg);

/// Set or overwrite a previously set 'disconnect' listener.
/// @param listener set NULL to reset
/// @param listener_arg is a pass-through argument passed to the listener
OBX_C_API void obx_sync_listener_disconnect(OBX_sync *sync, OBX_sync_listener_disconnect *listener,
                                            void *listener_arg);
...
/// @param listener_arg is a pass-through argument passed to the listener
OBX_C_API void
obx_sync_listener_login(OBX_sync *sync, OBX_sync_listener_login *listener, void *listener_arg);

/// Set or overwrite a previously set 'login failure' listener.
/// @param listener set NULL to reset
/// @param listener_arg is a pass-through argument passed to the listener
OBX_C_API void
obx_sync_listener_login_failure(OBX_sync *sync, OBX_sync_listener_login_failure *listener,
                                void *listener_arg);

/// Set or overwrite a previously set 'complete' listener - notifies when the latest sync has finished.
/// @param listener set NULL to reset
/// @param listener_arg is a pass-through argument passed to the listener
OBX_C_API void obx_sync_listener_complete(OBX_sync *sync, OBX_sync_listener_complete *listener,
                                          void *listener_arg);
...
/// @param listener_arg is a pass-through argument passed to the listener
OBX_C_API void
obx_sync_listener_change(OBX_sync *sync, OBX_sync_listener_change *listener, void *listener_arg);

/// Set or overwrite a previously set 'serverTime' listener - provides current time updates from the sync-server.
/// @param listener set NULL to reset
/// @param listener_arg is a pass-through argument passed to the listener
OBX_C_API void
obx_sync_listener_server_time(OBX_sync *sync, OBX_sync_listener_server_time *listener,
                              void *listener_arg);
...
/// @param listener_arg is a pass-through argument passed to the listener
OBX_C_API void
obx_sync_listener_msg_objects(OBX_sync *sync, OBX_sync_listener_msg_objects *listener,
                              void *listener_arg);
...
OBX_C_API obx_err
obx_sync_server_certificate_path(OBX_sync_server *server, const char *certificate_path);

/// Sets credentials for the server to accept. Use before obx_sync_server_start().
/// @param data may be NULL in combination with OBXSyncCredentialsType_NONE
OBX_C_API obx_err
obx_sync_server_credentials(OBX_sync_server *server, OBXSyncCredentialsType type, const void *data,
                            size_t size);
...
OBX_C_API obx_err
obx_sync_server_history_max_size_in_kb(OBX_sync_server *server, uint64_t max_in_kb,
...
/// @param listener_arg is a pass-through argument passed to the listener
OBX_C_API obx_err
obx_sync_server_listener_change(OBX_sync_server *server, OBX_sync_listener_change *listener,
                                void *listener_arg);
...
OBX_C_API const char *
obx_sync_server_stats_string(OBX_sync_server *server, bool include_zero_values);
...
/// @returns error code if an unexpected error occurred.
OBX_C_API obx_err
obx_sync_server_send_msg_objects(OBX_sync_server *server, OBX_sync_msg_objects_builder *message);
...
typedef void *OBX_custom_msg_server_func_create(uint64_t server_id, const char *url,
...
/// @param server_user_data User supplied data returned by the function that created the server
typedef bool OBX_custom_msg_server_func_client_connection_send_async(OBX_bytes_lazy *bytes,
...
/// Must be provided to implement a custom server. See notes on OBX_custom_msg_server_functions for more details.
/// @param server_user_data User supplied data returned by the function that created the server
typedef void OBX_custom_msg_server_func_client_connection_close(void *server_user_data,
                                                                void *connection_user_data);
...
OBX_C_API obx_err
obx_custom_msg_server_register(const char *protocol, OBX_custom_msg_server_functions *functions,
                               void *config_user_data);
...
OBX_C_API obx_err
obx_custom_msg_server_remove_client_connection(uint64_t server_id, uint64_t client_connection_id);
...
/// @returns OBX_ERROR_* in case the operation encountered an exceptional issue
OBX_C_API obx_err
obx_custom_msg_server_receive_message_from_client(uint64_t server_id, uint64_t client_connection_id,
...
typedef void *OBX_custom_msg_client_func_create(uint64_t client_id, const char *url,
                                                const char *cert_path,
                                                void *config_user_data);
...
/// Must be provided to implement a custom client. See notes on OBX_custom_msg_client_functions for more details.
/// @param client_user_data user supplied data returned by the function that created the client
typedef void OBX_custom_msg_client_func_disconnect(bool clear_outgoing_messages,
...
/// @returns Any other fitting error code (OBX_ERROR_*) if the protocol could not be registered
OBX_C_API obx_err
obx_custom_msg_client_register(const char *protocol, OBX_custom_msg_client_functions *functions,
                               void *config_user_data);
...
/// @returns OBX_ERROR_* in case the operation encountered an exceptional issue
OBX_C_API obx_err
obx_custom_msg_client_receive_message_from_server(uint64_t client_id, const void *message_data,
                                                  size_t message_size);
...
/// @returns OBX_ERROR_* in case the operation encountered an exceptional issue
OBX_C_API obx_err
obx_custom_msg_client_set_state(uint64_t client_id, OBXCustomMsgClientState state);
