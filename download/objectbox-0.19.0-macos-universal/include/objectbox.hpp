...
/// Note: what() typically contains a specific text about the error condition (sometimes helpful to resolve the issue).
    ...
    public:
        explicit DbException(const std::string &text, int code = 0) : Exception(text),
                                                                      code_(code) {}
...
/// @throws Exception using the given error (defaults to obx_last_error_code())
        [[noreturn]] void
        throwLastError(obx_err err = obx_last_error_code(), const char *contextPrefix = nullptr);
...
/// Unlike void* style bytes, this may represent allocated resources and/or bytes that are only produced on demand.
    ...
    public:
        ...
        BytesLazy(BytesLazy &&src)

        noexcept : cPtr_(src.cPtr_) {src.cPtr_ = nullptr;}
        ...
        void get(const void *&outBytes, size_t &outSize) const {
            if (cPtr_ == nullptr)
                throw IllegalStateException("This instance does not hold any bytes resources");
    ...
    using ObxLogCallback = std::function<void(OBXLogLevel logLevel, const char *text,
                                              size_t textSize)>;
...
///       Thus, you need to create a new Option object for each Store that is created.
    ...
    public:
        ...
        Options &
        validateOnOpenPages(size_t pageLimit, uint32_t flags = OBXValidateOnOpenPagesFlags_None) {
        ...
        /// single operation before AsyncQ starts. Note: this value should typically be low to keep latency low and
        /// prevent accumulating too much operations.
        Options &asyncPreTxnDelay(uint32_t delayMicros, uint32_t delay2Micros,
        ...
        Options &asyncPostTxnDelay(uint32_t delayMicros, uint32_t delay2Micros,
                                   size_t minQueueLengthForDelay2,
...
/// It's possible open multiple stores in different directories, e.g. at the same time.
    ...
    public:
        ...
        static size_t getDbFileSize(const std::string &directory) {
            return obx_db_file_size(directory.c_str());
        }
        ...
        explicit Store(Options &options)
                : Store(internal::checkedPtrOrThrow(obx_store_open(options.release()),
                                                    "Can not open store"), true) {}
        ...
        obx_schema_id getEntityTypeId(const char *entityName) const {
            ...
            if (typeId == 0)
                internal::throwIllegalStateException("No entity type found for name: ", entityName);
            return typeId;
        }
        ...
        obx_schema_id getPropertyId(obx_schema_id entityId, const char *propertyName) const {
            ...
            if (propertyId == 0)
                internal::throwIllegalStateException("No property found for name: ", propertyName);
            return propertyId;
        }
...
/// and actually commit.
    ...
    public:
        ...
        noexcept : mode_(source.mode_), cTxn_(source
        .cTxn_) { source.cTxn_ = nullptr; }
...
/// Internal cursor wrapper for convenience and RAII.
        ...
        public:
            ...
            CursorTx(CursorTx &&source)

            noexcept : tx_(std::move(source.tx_)), cCursor_(source
            .cCursor_) {
            ...
            obx_id seekToFirstId() {
                ...
                if (err != OBX_NOT_FOUND)
                    internal::checkErrOrThrow(
                            err);  // return 0 if not found (non-exceptional outcome)
                return id;
            }
...
std::vector<obx_id> idVectorOrThrow(OBX_id_array* cIds) {
    ...
    namespace {
        ...
        protected:
            ...
            friend obx_qb_cond
            internalApplyCondition(const QueryCondition &condition, OBX_query_builder *cqb,
                                   bool isRoot) {
            ...
            friend std::unique_ptr <QueryCondition>
            internalCopyAsPtr(const QueryCondition &condition) {
        ...
        public:
            QCGroup(bool isOr, std::unique_ptr <QueryCondition> &&a,
                    std::unique_ptr <QueryCondition> &&b)
        ...
        protected:
            ...
            obx_qb_cond applyTo(OBX_query_builder *cqb, bool isRoot) const override {
                if (conditions_.size() == 1)
                    return internalApplyCondition(*conditions_[0], cqb, isRoot);
        ...
        inline QCGroup obx::QueryCondition::operator&&(const QueryCondition &rhs) {
            return and_(rhs);
        }

        QCGroup obx::QueryCondition::or_(const QueryCondition &other) {
            return {true, copyAsPtr(), internalCopyAsPtr(other)};
        }
...
// using QCInt64, StringVector::contains query using QCString, etc.
        ...
        protected:
            ...
            [[noreturn]] void throwInvalidOperation() const {
                internal::throwIllegalStateException(
                        "Invalid condition - operation not supported: ",
        ...
        protected:
            std::unique_ptr <QueryCondition>
            copyAsPtr() const override { return QueryCondition::copyAsPtr(*this); };
            ...
            obx_qb_cond applyTo(OBX_query_builder *cqb, bool /*isRoot*/) const override {
                if (PropertyType == OBXPropertyType_String) {
                    if (op_ == QueryOp::Equal) {
                        ...
                        return obx_qb_not_equals_string(cqb, propId_, value_.c_str(),
                                                        caseSensitive_);
                    } else if (op_ == QueryOp::Less) {
                        return obx_qb_less_than_string(cqb, propId_, value_.c_str(),
                                                       caseSensitive_);
                    } else if (op_ == QueryOp::LessOrEq) {
                        return obx_qb_less_or_equal_string(cqb, propId_, value_.c_str(),
                                                           caseSensitive_);
                    } else if (op_ == QueryOp::Greater) {
                        return obx_qb_greater_than_string(cqb, propId_, value_.c_str(),
                                                          caseSensitive_);
                    } else if (op_ == QueryOp::GreaterOrEq) {
                        return obx_qb_greater_or_equal_string(cqb, propId_, value_.c_str(),
                                                              caseSensitive_);
                    } else if (op_ == QueryOp::StartsWith) {
                        return obx_qb_starts_with_string(cqb, propId_, value_.c_str(),
                                                         caseSensitive_);
                    } else if (op_ == QueryOp::EndsWith) {
                        return obx_qb_ends_with_string(cqb, propId_, value_.c_str(),
                                                       caseSensitive_);
                    ...
                    if (op_ == QueryOp::Contains) {
                        return obx_qb_any_equals_string(cqb, propId_, value_.c_str(),
                                                        caseSensitive_);
                    }
                }
                throwInvalidOperation();
        ...
        public:
            QCStringArray(obx_schema_id propId, QueryOp op, bool caseSensitive,
        ...
        protected:
            ...
            obx_qb_cond applyTo(OBX_query_builder *cqb, bool /*isRoot*/) const override {
                ...
                if (op_ == QueryOp::In) {
                    return obx_qb_in_strings(cqb, propId_, cvalues.data(), cvalues.size(),
                                             caseSensitive_);
                }
                throwInvalidOperation();
            }
        ...
        public:
            ...
            QCBytes(obx_schema_id propId, QueryOp op, const void *data, size_t size)
                    : QC(propId, op), value_(static_cast<const uint8_t *>(data),
        ...
        protected:
            ...
            obx_qb_cond applyTo(OBX_query_builder *cqb, bool /*isRoot*/) const override {
                if (op_ == QueryOp::Equal) {
                    ...
                    return obx_qb_greater_or_equal_bytes(cqb, propId_, value_.data(),
                                                         value_.size());
                }
                throwInvalidOperation();
            }
...
// enable_if_t missing in c++11 so let's have a shorthand here
        ...
        using EnableIfInteger =
                enable_if_t<T == OBXPropertyType_Int || T == OBXPropertyType_Long ||
                            T == OBXPropertyType_Short ||
                            T == OBXPropertyType_Byte || T == OBXPropertyType_Bool ||
                            T == OBXPropertyType_Date ||
                            T == OBXPropertyType_DateNano ||
                            (includingRelation && T == OBXPropertyType_Relation)>;

        template<OBXPropertyType T>
        using EnableIfIntegerOrRel = EnableIfInteger<T, true>;

        template<OBXPropertyType T>
        using EnableIfFloating = enable_if_t<
                T == OBXPropertyType_Float || T == OBXPropertyType_Double>;

        template<OBXPropertyType T>
        using EnableIfDate = enable_if_t<
                T == OBXPropertyType_Date || T == OBXPropertyType_DateNano>;

        constexpr OBXPropertyType
        typeless = static_cast<OBXPropertyType>(0);
    }  // namespace
...
/// Typeless property used as a base class for other types - sharing common conditions.
    ...
    public:
        explicit constexpr PropertyTypeless(obx_schema_id id)
...
/// Carries property information when used in the entity-meta ("underscore") class
    ...
    public:
        explicit constexpr Property(obx_schema_id id)

        noexcept : PropertyTypeless(id) {}
...
/// Carries property information when used in the entity-meta ("underscore") class
    ...
    public:
        ...
        QCBytes equals(std::vector <uint8_t> &&data) const {
            return {this->id_, QueryOp::Equal, std::move(data)};
        }

        QCBytes equals(const void *data, size_t size) const {
            return {this->id_, QueryOp::Equal, data, size};
        }

        QCBytes equals(const std::vector <uint8_t> &data) const {
            return equals(std::vector<uint8_t>(data));
        }

        QCBytes lessThan(std::vector <uint8_t> &&data) const {
            return {this->id_, QueryOp::Less, std::move(data)};
        }

        QCBytes lessThan(const void *data, size_t size) const {
            return {this->id_, QueryOp::Less, data, size};
        }

        QCBytes lessThan(const std::vector <uint8_t> &data) const {
            return lessThan(std::vector<uint8_t>(data));
        }

        QCBytes lessOrEq(std::vector <uint8_t> &&data) const {
            return {this->id_, QueryOp::LessOrEq, std::move(data)};
        }

        QCBytes lessOrEq(const void *data, size_t size) const {
            return {this->id_, QueryOp::LessOrEq, data, size};
        }

        QCBytes lessOrEq(const std::vector <uint8_t> &data) const {
            return lessOrEq(std::vector<uint8_t>(data));
        }

        QCBytes greaterThan(std::vector <uint8_t> &&data) const {
            return {this->id_, QueryOp::Greater, std::move(data)};
        }

        QCBytes greaterThan(const void *data, size_t size) const {
            return {this->id_, QueryOp::Greater, data, size};
        }

        QCBytes greaterThan(const std::vector <uint8_t> &data) const {
            return greaterThan(std::vector<uint8_t>(data));
        }
        ...
        QCBytes greaterOrEq(const void *data, size_t size) const {
            return {this->id_, QueryOp::GreaterOrEq, data, size};
        }

        QCBytes greaterOrEq(const std::vector <uint8_t> &data) const {
            return greaterOrEq(std::vector<uint8_t>(data));
        }
    };
...
/// Carries property information when used in the entity-meta ("underscore") class
    ...
    public:
        ...
        QCStringForStringVector
        contains(const std::string &value, bool caseSensitive = true) const {
            return contains(std::string(value), caseSensitive);
        }
    };
...
/// Carries property-based to-one relation information when used in the entity-meta ("underscore") class
    ...
    public:
        explicit constexpr RelationProperty(obx_schema_id id)
...
/// Carries to-many relation information when used in the entity-meta ("underscore") class
    ...
    public:
        explicit constexpr RelationStandalone(obx_schema_id id)
...
/// time.
    ...
    public:
        ...
        explicit QueryBuilderBase(Store &store, OBX_query_builder *ptr, bool isRoot)
                : store_(store), cQueryBuilder_(ptr), entityId_(obx_qb_type_id(cQueryBuilder_)),
                  isRoot_(isRoot) {
        ...
        QueryBuilderBase(QueryBuilderBase &&source)

        noexcept
                : store_(source.store_),
                cQueryBuilder_(source
        .cQueryBuilder_),
        entityId_(source
        .entityId_),
        isRoot_(source
        ...
        QueryBuilderBase &
        relationCount(obx_schema_id relationEntityId, obx_schema_id relationPropertyId,
                      uint32_t relationCount) {
            obx_qb_cond condition =
                    obx_qb_relation_count_property(cQueryBuilder_, relationEntityId,
                                                   relationPropertyId, relationCount);
        ...
        QueryBuilderBase &
        equalsString(obx_schema_id propertyId, const char *value, bool caseSensitive = true) {
            obx_qb_cond condition = obx_qb_equals_string(cQueryBuilder_, propertyId, value,
                                                         caseSensitive);
            if (condition == 0) internal::throwLastError();
            return *this;
        }
        ...
        QueryBuilderBase &
        notEqualsString(obx_schema_id propertyId, const char *value, bool caseSensitive = true) {
            obx_qb_cond condition = obx_qb_not_equals_string(cQueryBuilder_, propertyId, value,
                                                             caseSensitive);
            if (condition == 0) internal::throwLastError();
            return *this;
        }
...
/// To specify actual conditions, use obx_qb_*() methods with queryBuilder.cPtr() as the first argument.
    ...
    public:
        ...
        QueryBuilder<RangePropertyEntityT>
        linkTime(Property<RangePropertyEntityT, RangePropertyType> property) {
        ...
        template<typename RangePropertyEntityT, OBXPropertyType RangePropertyType,
                typename = EnableIfDate<RangePropertyType>>
        QueryBuilder<RangePropertyEntityT>
        linkTime(Property<RangePropertyEntityT, RangePropertyType> beginProperty,
                 Property<RangePropertyEntityT, RangePropertyType> endProperty) {
            return linkedQB<RangePropertyEntityT>(
                    obx_qb_link_time(cPtr(), entityId<RangePropertyEntityT>(), beginProperty.id(),
                                     endProperty.id()));
        }
        ...
        QueryBuilder<RelSourceEntityT> backlink(RelationProperty<RelSourceEntityT, EntityT> rel) {
            return linkedQB<RelSourceEntityT>(
                    obx_qb_backlink_property(cPtr(), entityId<RelSourceEntityT>(), rel.id()));
        }
...
/// Created by QueryBuilder and typically used with supplying a Cursor.
    ...
    public:
        ...
        explicit QueryBase(Store &store, OBX_query_builder *qb) : store_(store),
                                                                  cQuery_(obx_query(qb)) {
            internal::checkPtrOrThrow(cQuery_, "Can not build query");
        }

        /// Clones the query
        QueryBase(const QueryBase &query) : store_(query.store_),
                                            cQuery_(obx_query_clone(query.cQuery_)) {
            internal::checkPtrOrThrow(cQuery_, "Can not clone query");
        }

        QueryBase(QueryBase &&source)

        noexcept : store_(source.store_), cQuery_(source
        .cQuery_) {
        ...
        std::vector <obx_id> findIds() {
            return internal::idVectorOrThrow(obx_query_find_ids(cQuery_));
        }
        ...
        QueryBase &
        setParameter(obx_schema_id entityId, obx_schema_id propertyId, const char *value) {
            internal::checkErrOrThrow(obx_query_param_string(cQuery_, entityId, propertyId, value));
            return *this;
        }

        QueryBase &
        setParameter(obx_schema_id entityId, obx_schema_id propertyId, const std::string &value) {
...
/// Created by QueryBuilder and typically used with supplying a Cursor.
    ...
    public:
        ...
        std::unique_ptr <EntityT> findFirst() {
            return findSingle < std::unique_ptr <
                   EntityT >> (obx_query_find_first, EntityT::_OBX_MetaInfo::newFromFlatBuffer);
        }
        ...
        std::unique_ptr <EntityT> findUnique() {
            return findSingle < std::unique_ptr <
                   EntityT >> (obx_query_find_unique, EntityT::_OBX_MetaInfo::newFromFlatBuffer);
        }
    ...
    std::optional<EntityT> findUniqueOptional() {
        ...
        template<
                ...
                typename = enable_if_t<PropertyType == OBXPropertyType_String ||
                                       PropertyType == OBXPropertyType_StringVector>>
        ...
        template<
                ...
                typename = enable_if_t<PropertyType == OBXPropertyType_String ||
                                       PropertyType == OBXPropertyType_StringVector>>
        Query &
        setParameter(Property<PropertyEntityT, PropertyType> property, const std::string &value) {
        ...
        Query &setParameter(Property<PropertyEntityT, OBXPropertyType_String> property,
                            ...
                            size_t count) {
            obx_err err = obx_query_param_strings(cQuery_, entityId<PropertyEntityT>(),
                                                  property.id(), values, count);
            internal::checkErrOrThrow(err);
            return *this;
        }
        ...
        Query &setParameter(Property<PropertyEntityT, OBXPropertyType_Bool> property, bool value) {
            internal::checkErrOrThrow(
                    obx_query_param_int(cQuery_, entityId<PropertyEntityT>(), property.id(),
                                        value));
            return *this;
        }
        ...
        /// Change previously set condition value in an existing query - this improves reusability of the query object.
        template<typename PropertyEntityT, OBXPropertyType PropertyType, typename = EnableIfIntegerOrRel<PropertyType>>
        Query &setParameters(Property<PropertyEntityT, PropertyType> property, int64_t valueA,
                             int64_t valueB) {
            obx_err err = obx_query_param_2ints(cQuery_, entityId<PropertyEntityT>(), property.id(),
                                                valueA, valueB);
            internal::checkErrOrThrow(err);
            return *this;
        }
        ...
        template<typename PropertyEntityT, OBXPropertyType PropertyType,
                typename = enable_if_t<PropertyType == OBXPropertyType_Long ||
                                       PropertyType == OBXPropertyType_Relation>>
        Query &setParameter(Property<PropertyEntityT, PropertyType> property,
                            const std::vector <int64_t> &values) {
            obx_err err =
                    obx_query_param_int64s(cQuery_, entityId<PropertyEntityT>(), property.id(),
        ...
        Query &setParameter(Property<PropertyEntityT, PropertyType> property,
            ...
            obx_err err =
                    obx_query_param_int32s(cQuery_, entityId<PropertyEntityT>(), property.id(),
                                           values.data(), values.size());
            internal::checkErrOrThrow(err);
            return *this;
        }
        ...
        Query &setParameter(Property<PropertyEntityT, PropertyType> property, double value) {
            internal::checkErrOrThrow(
                    obx_query_param_double(cQuery_, entityId<PropertyEntityT>(), property.id(),
                                           value));
            return *this;
        }
        ...
        /// Change previously set condition value in an existing query - this improves reusability of the query object.
        template<typename PropertyEntityT, OBXPropertyType PropertyType, typename = EnableIfFloating<PropertyType>>
        Query &setParameters(Property<PropertyEntityT, PropertyType> property, double valueA,
                             double valueB) {
            obx_err err = obx_query_param_2doubles(cQuery_, entityId<PropertyEntityT>(),
                                                   property.id(), valueA, valueB);
            internal::checkErrOrThrow(err);
            return *this;
        }
        ...
        Query &setParameter(Property<PropertyEntityT, OBXPropertyType_ByteVector> property,
                            const void *value,
                            size_t size) {
            obx_err err = obx_query_param_bytes(cQuery_, entityId<PropertyEntityT>(), property.id(),
                                                value, size);
            internal::checkErrOrThrow(err);
            return *this;
        }
    ...
    private:
        ...
        RET findSingle(obx_err nativeFn(OBX_query *, const void **, size_t *),
                       T fromFlatBuffer(const void *, size_t)) {
            OBX_VERIFY_STATE(cQuery_);
...
// ensure to add the line "#define OBX_CPP_FILE" before including this file in (exactly) one of your .cpp/.cc files.
        ...
        threadLocalFbbDirty();  ///< Not cleared, thus potentially "dirty" fbb
#endif
...
/// Serves as the basis for Box, but can also be used as "lower-level" box with some restrictions on the functionality.
    ...
    public:
        BoxTypeless(Store &store, obx_schema_id entityTypeId)
                : store_(store), cBox_(obx_box(store.cPtr(), entityTypeId)),
                  entityTypeId_(entityTypeId) {
            if (cBox_ == nullptr) {
                std::string msg =
                        "Can not create box for entity type ID " + std::to_string(entityTypeId_);
...
/// Box instances are thread-safe and light-weight wrappers around C-boxes, which are cached internally (see obx_box()).
    ...
    std::vector<std::optional<EntityT>> getOptional(const std::vector<obx_id>& ids) {
        ...
        size_t put(std::vector <EntityT> &objects, std::vector <obx_id> *outIds = nullptr,
        ...
        size_t
        put(std::vector <std::unique_ptr<EntityT>> &objects, std::vector <obx_id> *outIds = nullptr,
            OBXPutMode mode = OBXPutMode_PUT) {
            return putMany(objects, outIds, mode);
        }
    ...
    size_t put(std::vector<std::optional<EntityT>>& objects, std::vector<obx_id>* outIds = nullptr,
        ...
        template<typename SourceEntityT, typename TargetEntityT>
        std::vector <obx_id>
        backlinkIds(RelationProperty<SourceEntityT, TargetEntityT> toOneRel, obx_id objectId) {
            ...
            return internal::idVectorOrThrow(
                    obx_box_get_backlink_ids(cBox_, toOneRel.id(), objectId));
        }
        ...
        template<typename SourceEntityT, typename TargetEntityT>
        void standaloneRelReplace(RelationStandalone<SourceEntityT, TargetEntityT> toManyRel,
        ...
        void standaloneRelPut(RelationStandalone<EntityT, TargetEntityT> toManyRel,
                              obx_id sourceObjectId,
                              obx_id targetObjectId) {
            internal::checkErrOrThrow(
                    obx_box_rel_put(cBox_, toManyRel.id(), sourceObjectId, targetObjectId));
        }
        ...
        /// @param targetObjectId identifies an object from a target box (as per the relation definition)
        template<typename TargetEntityT>
        void standaloneRelRemove(RelationStandalone<EntityT, TargetEntityT> toManyRel,
                                 obx_id sourceObjectId,
                                 obx_id targetObjectId) {
            internal::checkErrOrThrow(
                    obx_box_rel_remove(cBox_, toManyRel.id(), sourceObjectId, targetObjectId));
        }
        ...
        std::vector <obx_id>
        standaloneRelIds(RelationStandalone<SourceEntityT, EntityT> toManyRel, obx_id objectId) {
        ...
        template<typename TargetEntityT>
        std::vector <obx_id>
        standaloneRelBacklinkIds(RelationStandalone<EntityT, TargetEntityT> toManyRel,
                                 obx_id objectId) {
            return internal::idVectorOrThrow(
                    obx_box_rel_get_backlink_ids(cBox_, toManyRel.id(), objectId));
        }
        ...
        bool timeSeriesMinMax(obx_id *outMinId, int64_t *outMinValue, obx_id *outMaxId,
                              int64_t *outMaxValue) {
        ...
        bool timeSeriesMinMax(int64_t rangeBegin, int64_t rangeEnd, obx_id *outMinId,
                              int64_t *outMinValue,
                              obx_id *outMaxId, int64_t *outMaxValue) {
            obx_err err =
                    obx_box_ts_min_max_range(cBox_, rangeBegin, rangeEnd, outMinId, outMinValue,
    ...
    private:
        ...
        size_t putMany(Vector &objects, std::vector <obx_id> *outIds, OBXPutMode mode) {
            ...
            for (auto &object: objects) {
                ...
                if (outIds)
                    outIds->push_back(
                            id);  // always include in outIds even if the item wasn't present (id == 0)
                if (id) count++;
            }
        ...
        obx_id
        cursorPut(CursorTx &cursor, flatbuffers::FlatBufferBuilder &fbb, const EntityT &object,
                  OBXPutMode mode) {
            EntityBinding::toFlatBuffer(fbb, object);
            obx_id id = obx_cursor_put_object4(cursor.cPtr(), fbb.GetBufferPointer(), fbb.GetSize(),
                                               mode);
            internal::checkIdOrThrow(id);
            return id;
        }

        obx_id cursorPut(CursorTx &cursor, flatbuffers::FlatBufferBuilder &fbb, EntityT &object,
                         OBXPutMode mode) {
            // Using a const_cast to add "const" only to identify the method overload:
            // NOLINTNEXTLINE(cppcoreguidelines-pro-type-const-cast)
        ...
        obx_id cursorPut(CursorTx &cursor, flatbuffers::FlatBufferBuilder &fbb,
                         const std::unique_ptr <EntityT> &object,
                         OBXPutMode mode) {
...
/// AsyncBox provides asynchronous ("happening on the background") database manipulation.
    ...
    class AsyncBox {
        ...
        /// Creates a shared AsyncBox instance.
        AsyncBox(Store &store, bool owned, OBX_async *ptr) : store_(store), created_(owned),
                                                             cAsync_(ptr) {
    ...
    public:
        ...
        AsyncBox(Store &store, uint64_t enqueueTimeoutMillis)
                : AsyncBox(store, true,
                           obx_async_create(store.box<EntityT>().cPtr(), enqueueTimeoutMillis)) {}

        /// Move constructor
        AsyncBox(AsyncBox &&source)

        noexcept : store_(source.store_), cAsync_(source
        .cAsync_),
        created_(source
        .created_) {
...
/// @brief Structural/behavioral options for a tree passed during tree creation.
    ...
    public:
        ...
        TreeOptions(TreeOptions &&source)

        noexcept : cOptions_(source.cOptions_) {source.cOptions_ = nullptr;}
...
/// Parameter to AsyncTreePutCallback, which is passed to Tree::putAsync().
    struct AsyncTreePutResult {
        ...
        [[noreturn]] void throwException() {
            internal::throwError(status, "Async tree put failed: " + errorMessage);
        }
    };
...
/// Data is accessed via TreeCursor.
    ...
    public:
        ...
        Tree(Store &store, TreeOptions &options)
                : cTree_(obx_tree(store.cPtr(), options.cOptions_)),
                  pathDelimiter_(options.pathDelimiter_) {
        ...
        noexcept : cTree_(source.cTree_), pathDelimiter_(source
        .pathDelimiter_) {
        ...
        ///        AsyncTreePutResult.
        void putAsync(const char *path, void *data, size_t size, OBXPropertyType type,
        ...
        void putAsyncRawCallback(const char *path, void *data, size_t size, OBXPropertyType type,
                                 void *metadata = nullptr,
                                 size_t metadata_size = 0, OBXPutMode dataPutMode = OBXPutMode_PUT,
...
/// Contains meta data about (data) leaves: full path in the tree, ID, and property type.
    ...
    public:
        ...
        const char *leafPathCString(size_t index) {
            return obx_tree_leaves_info_path(cLeavesInfo_, index);
        }
        ...
        OBXPropertyType leafPropertyType(size_t index) {
            return obx_tree_leaves_info_type(cLeavesInfo_, index);
        }
...
/// thread.
    ...
    public:
        TreeCursor(Tree &tree, Transaction *tx) : cCursor_(
                obx_tree_cursor(tree.cTree_, tx ? tx->cPtr() : nullptr)) {
        ...
        /// Move constructor "stealing" the C resource from the source
        TreeCursor(TreeCursor &&source)

        noexcept : cCursor_(source.cCursor_) {source.cCursor_ = nullptr;}
        ...
        bool get(const char *path, const void **data, size_t *size, const void **metadata = nullptr,
            ...
            obx_err err = obx_tree_cursor_get_raw(cCursor_, path, data, size, metadata,
                                                  metadataSize);
            if (err == OBX_NOT_FOUND) return false;
            internal::checkErrOrThrow(err);
            return true;
        ...
        const char *getLeafPathCString(obx_id leafId) {
            return obx_tree_cursor_get_leaf_path(cCursor_, leafId);
        }
        ...
        LeavesInfo getLeavesInfo(const char *path = nullptr) {
            OBX_tree_leaves_info *cLeavesInfo = obx_tree_cursor_get_child_leaves_info(cCursor_,
                                                                                      path);
            internal::checkPtrOrThrow(cLeavesInfo, "Could not create leaves info");
            return LeavesInfo(cLeavesInfo);
        }

        inline LeavesInfo getLeavesInfo(const std::string &path) {
            return getLeavesInfo(path.c_str());
        }
        ...
        TreePutResult put(const char *path, void *data, size_t size, OBXPropertyType type,
                          obx_id *outId = nullptr,
                          void *metadata = nullptr, size_t metadata_size = 0,
                          OBXPutMode dataPutMode = OBXPutMode_PUT) {
            obx_err err =
                    obx_tree_cursor_put_raw(cCursor_, path, data, size, type, outId, metadata,
                                            metadata_size, dataPutMode);
