...
namespace obx {
    ...
    public:
        SyncCredentials(OBXSyncCredentialsType type, std::vector <uint8_t> &&data) : type_(type),
                                                                                     data_(std::move(
                                                                                             data)) {}

        SyncCredentials(OBXSyncCredentialsType type, const std::string &data) : type_(type),
                                                                                data_(data.size()) {
            static_assert(sizeof(data_[0]) == sizeof(data[0]),
                          "Can't directly copy std::string to std::vector<uint8_t>");
            if (!data.empty()) memcpy(data_.data(), data.data(), data.size() * sizeof(data[0]));
        }

        static SyncCredentials none() {
            return SyncCredentials(OBXSyncCredentialsType::OBXSyncCredentialsType_NONE,
                                   std::vector < uint8_t > {});
        }

        static SyncCredentials sharedSecret(std::vector <uint8_t> &&data) {
            return SyncCredentials(OBXSyncCredentialsType::OBXSyncCredentialsType_SHARED_SECRET,
                                   std::move(data));
        }
...
/// Listens to login events on a sync client.
    ...
    public:
        ...
        virtual void loginFailed(OBXSyncCode)
...
/// Listens to sync time information events on a sync client.
    ...
    public:
        ...
        virtual void serverTime(TimePoint time)
...
/// @note this may affect performance. Use SyncClientCompletionListener for the general synchronization-finished check.
    ...
    public:
        ...
        virtual void changed(const std::vector <SyncChange> &changes)
    ...
    private:
        ...
        void changed(const OBX_sync_change_array *cChanges)
...
/// Listens to all possible sync events. See each base abstract class for detailed information.
    ...
    public:
        ...
        virtual void received(const OBX_sync_msg_objects *cObjects)
...
/// using SyncClient::send() or SyncServer::send().
    ...
    public:
        ...
        SyncObjectsMessageBuilder(SyncObjectsMessageBuilder &&source)

        noexcept : cBuilder_(source.cBuilder_) {
        ...
        void add(OBXSyncObjectType type, const void *data, size_t size, uint64_t id = 0) {
            internal::checkErrOrThrow(
                    obx_sync_msg_objects_builder_add(cBuilder_, type, data, size, id));
        }
...
/// Sync client is used to provide ObjectBox Sync client capabilities to your application.
    ...
    public:
        ...
        explicit SyncClient(Store &store, const std::string &serverUrl,
        ...
        void setCredentials(const SyncCredentials &creds) {
            obx_err err = obx_sync_credentials(cPtr(), creds.type_,
                                               creds.data_.empty() ? nullptr : creds.data_.data(),
                                               creds.data_.size());
        ...
        bool triggerReconnect() {
            return internal::checkSuccessOrThrow(obx_sync_trigger_reconnect(cPtr()));
        }
        ...
        void setHeartbeatInterval(std::chrono::milliseconds interval) {
            internal::checkErrOrThrow(
                    obx_sync_heartbeat_interval(cPtr(), static_cast<uint64_t>(interval.count())));
        }
        ...
        void maxMessagesInFlight(int value) {
            internal::checkErrOrThrow(obx_sync_max_messages_in_flight(cPtr(), value));
        }
        ...
        bool requestUpdates(bool subscribeForFuturePushes) {
            return internal::checkSuccessOrThrow(
                    obx_sync_updates_request(cPtr(), subscribeForFuturePushes));
        }

        /// Cancel updates from the server so that it will stop sending updates.
        /// @see updatesRequest()
        bool cancelUpdates() {
            return internal::checkSuccessOrThrow(obx_sync_updates_cancel(cPtr()));
        }
        ...
        void setLoginListener(std::shared_ptr <SyncClientLoginListener> listener) {
            ...
            if (listener) {
                ...
                obx_sync_listener_login(
                        cPtr(),
                        [](void *userData) { static_cast<SyncClientLoginListener *>(userData)->loggedIn(); },
                        listenerPtr);
        ...
        void setCompletionListener(std::shared_ptr <SyncClientCompletionListener> listener) {
            ...
            if (listener) {
                ...
                obx_sync_listener_complete(
                        cPtr(),
                        [](void *arg) { static_cast<SyncClientCompletionListener *>(arg)->updatesCompleted(); },
        ...
        void setConnectionListener(std::shared_ptr <SyncClientConnectionListener> listener) {
            ...
            if (listener) {
                ...
                obx_sync_listener_connect(
                        cPtr(),
                        [](void *userData) { static_cast<SyncClientConnectionListener *>(userData)->connected(); },
                        listenerPtr);
                obx_sync_listener_disconnect(
                        cPtr(),
                        [](void *userData) { static_cast<SyncClientConnectionListener *>(userData)->disconnected(); },
                        listenerPtr);
            }
        }
                ...
                obx_sync_listener_server_time(
                        ...
                        [](void *arg, int64_t timestampNs) {
                            static_cast<SyncClientTimeListener *>(arg)->serverTime(
                                    SyncClientTimeListener::TimePoint(
        ...
        void setListener(std::shared_ptr <SyncClientListener> listener) {
            ...
            if (listener) {
                ...
                obx_sync_listener_login(
                        cPtr(),
                        [](void *arg) { static_cast<SyncClientListener *>(arg)->loggedIn(); },
                        listenerPtr);
                obx_sync_listener_login_failure(
                        cPtr(), [](void *arg, OBXSyncCode code) {
                            static_cast<SyncClientListener *>(arg)->loginFailed(code);
                        },
                        listenerPtr);
                obx_sync_listener_complete(
                        cPtr(),
                        [](void *arg) { static_cast<SyncClientListener *>(arg)->updatesCompleted(); },
                        listenerPtr);
                obx_sync_listener_connect(
                        cPtr(),
                        [](void *arg) { static_cast<SyncClientListener *>(arg)->connected(); },
                        listenerPtr);
                obx_sync_listener_disconnect(
                        cPtr(),
                        [](void *arg) { static_cast<SyncClientListener *>(arg)->disconnected(); },
                        listenerPtr);
                obx_sync_listener_server_time(
                        cPtr(),
                        [](void *arg, int64_t timestampNs) {
    ...
    protected:
        ...
        void removeCompletionListener(bool evenIfEmpty = false) {
            std::shared_ptr <SyncClientCompletionListener> listener = std::move(
                    listeners_.complete);
            if (listener || evenIfEmpty) {
...
/// Start building a sync client using client() and connect to a remote server.
    ...
    public:
        ...
        static std::shared_ptr <SyncClient> client(Store &store, const std::string &serverUrl,
            ...
            if (store.syncClient_)
                throw IllegalStateException("Only one sync client can be active for a store");
...
/// Note that you need a special sync edition, which includes the server components. Check https://objectbox.io/sync/.
    ...
    public:
        ...
        SyncServer(SyncServer &&source)

        noexcept : cPtr_(source.cPtr_) {
        ...
        void setCredentials(const SyncCredentials &creds) {
            internal::checkErrOrThrow(obx_sync_server_credentials(
                    cPtr(), creds.type_, creds.data_.empty() ? nullptr : creds.data_.data(),
                    creds.data_.size()));
        }
        ...
        void setHistoryMaxSizeKb(uint64_t maxSizeKb, uint64_t targetSizeKb = 0) {
            internal::checkErrOrThrow(
                    obx_sync_server_history_max_size_in_kb(cPtr(), maxSizeKb, targetSizeKb));
        }
        ...
        std::string url() {
            return internal::checkedPtrOrThrow(obx_sync_server_url(cPtr()),
                                               "Can't get SyncServer bound URL");
        }
        ...
        std::string statsString(bool includeZeroValues = true) {
            ...
            return internal::checkedPtrOrThrow(serverStatsString,
...
/// Also, it provides makeFunctions() and registerProtocol().
    ...
    public:
        ...
        static void *
        delegateCreate(uint64_t clientId, const char *url, const char *certPath, void *userConfig) {
        ...
        static bool delegateConnect(void *clientUserData) {
            return sharedPtr(clientUserData)->connect();
        }
        ...
        static void registerProtocol(const char *protocol, void *configUserData = nullptr) {
            ...
            internal::checkErrOrThrow(
                    obx_custom_msg_client_register(protocol, &functions, configUserData));
        }
    };
...
///       implementor is aware of all the interactions and thus shall explicitly provide at least empty implementations.
    ...
    public:
        ...
        inline bool forwardReceivedMessageFromServer(const void *messageData, size_t messageSize) {
            obx_err err = obx_custom_msg_client_receive_message_from_server(id_, messageData,
                                                                            messageSize);
...
/// Also, it provides makeFunctions() and registerProtocol().
    ...
    public:
        ...
        static void *delegateCreate(uint64_t serverId, const char *url, const char *certPath,
                                    void *configUserData) {
        ...
        static void registerProtocol(const char *protocol, void *configUserData = nullptr) {
            ...
            internal::checkErrOrThrow(
                    obx_custom_msg_server_register(protocol, &functions, configUserData));
        }
    };
...
/// Subclasses represent a connection of a custom server.
    ...
    public:
        explicit AbstractCustomMsgConnection(uint64_t serverId, uint64_t id = 0) : serverId_(
                serverId), id_(id) {}
...
///       implementor is aware of all the interactions and thus shall explicitly provide at least empty implementations.
    ...
    public:
        ...
        inline uint64_t
        addConnection(const std::shared_ptr <AbstractCustomMsgConnection> &connection) {
            if (!connection) throw IllegalArgumentException("No connection was provided");
            auto delegate =
                    std::unique_ptr<CustomMsgConnectionDelegate>(
                            new CustomMsgConnectionDelegate(id(), connection, true));
            uint64_t connectionId = obx_custom_msg_server_add_client_connection(id(),
                                                                                delegate.get());
        ...
        /// @returns false in case the operation encountered an issue.
        /// @throws Exception in case the operation encountered an exceptional issue
        inline bool forwardReceivedMessageFromClient(uint64_t connectionId, const void *messageData,
                                                     size_t messageSize) {
            obx_err err = obx_custom_msg_server_receive_message_from_client(id(), connectionId,
                                                                            messageData,
                                                                            messageSize);
            return internal::checkSuccessOrThrow(err);
        }
